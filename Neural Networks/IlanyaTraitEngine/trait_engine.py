...
from IlanyaTraitEngine.trait_transformer import TraitTransformer, TraitTransformerConfig
from IlanyaTraitEngine.trait_data import TraitData, TraitMatrix, TraitVector
from IlanyaTraitEngine.trait_types import TraitType, TraitCategory, TRAIT_METADATA, TraitDimension, IDENTITY_PROTECTED_TRAITS, PERMANENTLY_PROTECTED_TRAITS, PARTIALLY_PROTECTED_TRAITS, FULLY_EVOLVABLE_TRAITS
from IlanyaTraitEngine.trait_state import TraitState, CognitiveState
...
class TraitEngine:
    ...
    def _compute_identity_preservation_loss(self, input_traits: torch.Tensor, 
        ...
        """
        from IlanyaTraitEngine.trait_types import PERMANENTLY_PROTECTED_TRAITS, PARTIALLY_PROTECTED_TRAITS, TraitType
    ...
    def _prepare_trait_tensors(self, trait_data: TraitData) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Create a mapping from trait types to their indices
        trait_type_to_index = {trait_type: i for i, trait_type in enumerate(TraitType)}
        ...
        for i, (trait_type, trait_vector) in enumerate(trait_data.trait_matrix.traits.items()):
            ...
            # Get trait index safely
            if trait_type in trait_type_to_index:
                trait_indices[0, i] = trait_type_to_index[trait_type]
            else:
                # If trait type is not in the enum, use a default index
                trait_indices[0, i] = 0
    ...
    def _process_network_outputs(self, outputs: Dict[str, torch.Tensor], 
        ...
        # Get the actual number of traits from the output
        num_traits_output = trait_predictions.shape[0]
        num_traits_input = len(original_data.trait_matrix.traits)
        
        # Use the smaller of the two to avoid index errors
        num_traits = min(num_traits_output, num_traits_input)
        
        # Convert predictions back to trait data format
        predicted_traits = {}
        for i, (trait_type, _) in enumerate(list(original_data.trait_matrix.traits.items())[:num_traits]):
            if i < num_traits_output:
        ...
        return {
            ...
            'evolution_signals': evolution_signals[:num_traits] if len(evolution_signals) > num_traits else evolution_signals,
            'interaction_weights': interaction_weights[:num_traits, :num_traits] if interaction_weights.shape[0] > num_traits else interaction_weights,
    ...
    def _apply_trait_evolution(self, trait_data: TraitData, 
        ...
        """
        from IlanyaTraitEngine.trait_types import PERMANENTLY_PROTECTED_TRAITS, PARTIALLY_PROTECTED_TRAITS, FULLY_EVOLVABLE_TRAITS
        
        evolved_traits = {}
        
        # Ensure evolution signals array is large enough
        num_traits = len(trait_data.trait_matrix.traits)
        if len(evolution_signals) < num_traits:
            # Pad with zeros if evolution signals are too short
            evolution_signals = np.pad(evolution_signals, (0, num_traits - len(evolution_signals)), 'constant')
        
        for i, (trait_type, trait_vector) in enumerate(trait_data.trait_matrix.traits.items()):
            evolution_signal = evolution_signals[i] if i < len(evolution_signals) else 0.0
            current_value = trait_vector.value
